...
public class LoginPanel extends JPanel {
	...
	public LoginPanel() {
		...
		RegisterButton.addActionListener(e -> {
			UIController.getUnicaInstancia().showRegister(); 
			this.reset();
			});
		this.add(RegisterButton);
		
		textEmail = new JTextField();
		textEmail.setBounds(370, 160, 285, 30);
		textEmail.setBackground(this.Gray);
		...
		textEmail.addFocusListener(new FocusAdapter() {
            ...
            public void focusGained(FocusEvent e) {
                if (textEmail.getForeground().equals(Color.RED)) {
                	textEmail.setText(""); // Borrar el mensaje de error
                	textEmail.setForeground(Color.WHITE); // Restaurar color normal
                }
            }
        });
		this.add(textEmail);
		...
		lblEmail.setBounds(370, 142, 50, 20);
		...
		lblBanner.setIcon(new ImageIcon(LoginPanel.class.getResource("/assets/LoginBanner.png")));
	...
	public void doLogin() {
    	...
    	if(success) UIController.getUnicaInstancia().doLogin(email, passwd);
	...
	public void errors(byte code) {
		...
		case 1: {
			...
			this.textEmail.setText("invalid email");
			break;
		}
		case 2: {
		...
		case 4: {
			//System.err.println("[Error] : El valor no es una contraseña válida.");
			this.textEmail.setForeground(Color.RED);
			this.textEmail.setText("invalid login");
			this.password1.setForeground(Color.RED);
